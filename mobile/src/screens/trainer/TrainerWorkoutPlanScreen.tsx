...
const TrainerWorkoutPlanScreen = ({ navigation }: any) => {
  ...
  const fetchMembersWithWorkouts = useCallback(async () => {
    try {
      ...
      console.error('Error fetching members for workout plans:', err);
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
    ...
    <SafeAreaView edges={['top']} style={{ flex: 1, backgroundColor: '#FAFAFA' }}>
      <View className="flex-1 px-5 pt-4">
        ...
        <View className="flex-row justify-between items-center mb-4">
          <View>
            <Text className="text-2xl font-black text-[#1F2937]">Workout Plans</Text>
            <Text className="text-xs text-slate-500 mt-0.5">Build & assign routines to clients</Text>
          </View>
          <TouchableOpacity
            ...
            renderItem={({ item }) => {
              ...
              return (
                <View className="bg-white border border-slate-200 rounded-2xl p-4 mb-3 shadow-sm flex-row justify-between items-center">
                  ...
                  <TouchableOpacity
                    onPress={() => navigation.navigate('CreateWorkoutPlan', { preselectedMemberId: item._id || item.id })}
                    className="bg-indigo-50 border border-indigo-200 px-3 py-2 rounded-xl"
                  >
                    <Text className="text-indigo-600 text-xs font-bold">Assign Workout 💪</Text>
                  </TouchableOpacity>
                </View>
              );
            }}
