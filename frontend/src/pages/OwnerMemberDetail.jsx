  ...
  if (loading) {
    return (
      <div className="main-content flex-center" style={{ minHeight: '60vh' }}>
        <div className="text-slate-400 text-sm">Loading user file details...</div>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="p-6 md:p-8 max-w-7xl mx-auto w-full">
        ...
        <Link to="/owner/members" className="inline-flex items-center gap-2 text-indigo-400 hover:text-indigo-300 text-sm font-semibold">
          <ArrowLeft size={16} />
    ...
    <div className="p-6 md:p-8 max-w-7xl mx-auto w-full space-y-8">
      ...
      <div>
        ...
        <h2 className="text-3xl font-extrabold text-white">
          Member <span className="bg-gradient-to-r from-indigo-400 to-cyan-400 bg-clip-text text-transparent">Profile Folder</span>
        </h2>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        {/* Right Column: Attendance & Payment history listings */}
        <div className="lg:col-span-2 space-y-6">
          
          {/* Payment History Table (Table-based) */}
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 shadow-lg">
            ...
            {payments.length === 0 ? (
              <div className="text-sm text-slate-500 py-4">No invoicing transaction records logged.</div>
            ) : (
                ...
                <table className="w-full text-left border-collapse text-xs">
                  ...
                  <tbody>
                    {payments.map((payment) => (
                      <tr key={payment._id} className="border-b border-slate-950 hover:bg-slate-800/30 transition-colors">
                        <td className="py-3 px-3 text-slate-300">
                          {payment.payment_date ? new Date(payment.payment_date).toLocaleDateString() : 'N/A'}
                        </td>
                        <td className="py-3 px-3 font-bold text-slate-200">${payment.amount}</td>
                        <td className="py-3 px-3 text-slate-400">{new Date(payment.due_date).toLocaleDateString()}</td>
                        <td className="py-3 px-3 uppercase text-slate-500">{payment.payment_method}</td>
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 shadow-lg">
            ...
            {attendance.length === 0 ? (
              <div className="text-sm text-slate-500 py-4">No attendance sessions tracked.</div>
            ) : (
              <div className="overflow-x-auto">
                <table className="w-full text-left border-collapse text-xs">
                  <thead>
...
export default OwnerMemberDetail;
